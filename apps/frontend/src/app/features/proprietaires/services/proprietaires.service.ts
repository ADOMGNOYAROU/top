...
export class ProprietairesService {
  ...
  getAllProprietaires(): Observable<Proprietaire[]> {
    return this.http
      .get<Proprietaire[]>(this.apiUrl)
      .pipe(catchError(() => of(this.getMockProprietaires())));
  }
  ...
  getProprietaireById(id: string): Observable<Proprietaire> {
    return this.http
      .get<Proprietaire>(`${this.apiUrl}/${id}`)
      .pipe(
        catchError(() =>
          of(this.getMockProprietaires().find((p) => p.id === id)!),
        ),
      );
  }
  ...
  updateProprietaire(
    ...
    return this.http
      .put<Proprietaire>(`${this.apiUrl}/${id}`, proprietaire)
      .pipe(
        catchError(() => {
    ...
    return this.getAllProprietaires().pipe(
      map((proprietaires) => {
        return proprietaires.filter((proprietaire) => {
          if (filters.statut && proprietaire.statut !== filters.statut)
            return false;
          if (
            filters.ville &&
          ...
          if (filters.recherche) {
            ...
            const matchPrenoms = proprietaire.prenoms
              .toLowerCase()
              .includes(search);
            const matchEmail = proprietaire.email
              ?.toLowerCase()
              .includes(search);
