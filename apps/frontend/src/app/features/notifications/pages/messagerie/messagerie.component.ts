              ...
              <div class="divide-y divide-gray-200">
                @for (conversation of conversations; track conversation.id) {
                  <div
                    ...
                    (keydown.enter)="selectionnerConversation(conversation)"
                    (keydown.space)="selectionnerConversation(conversation)"
                    role="button"
                    tabindex="0"
                  ...
                  >
                    <div class="flex items-start gap-4">
                      <div
                        class="w-12 h-12 bg-primary rounded-full flex items-center justify-center text-white font-semibold flex-shrink-0"
                      >
                      ...
                      @if (conversation.nonLus > 0) {
                        <div
                          class="w-6 h-6 bg-primary rounded-full flex items-center justify-center text-white text-xs font-semibold flex-shrink-0"
                        >
                          {{ conversation.nonLus }}
                        </div>
                      }
          ...
          <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  <h3 class="font-semibold text-gray-900">
                    {{ conversationSelectionnee.locataireNom }}
                  </h3>
                  <p class="text-sm text-gray-600">
                    {{ conversationSelectionnee.bienTitre }}
                  </p>
                </div>
              </div>
          ...
          >
            <div class="bg-white rounded-xl p-6 max-w-md w-full mx-4">
              ...
              <form
                ...
                (ngSubmit)="creerConversation()"
                class="space-y-4"
              >
                <div>
                  <label
                    class="block text-sm font-medium text-gray-700 mb-2"
                    for="messagerie-locataire"
                    >Locataire</label
                  >
                  <select
                    id="messagerie-locataire"
...
export class MessagerieComponent implements OnInit {
  ...
  messageForm: ReturnType<MessagerieComponent["buildMessageForm"]>;
  newConversationForm: ReturnType<
    MessagerieComponent["buildNewConversationForm"]
  >;
  ...
  constructor() {
    this.messageForm = this.buildMessageForm();
    this.newConversationForm = this.buildNewConversationForm();
  }
  ...
  private buildNewConversationForm() {
    return this.fb.nonNullable.group({
    ...
    this.messages = [
      {
        ...
        contenu:
          "Bonjour, je voulais savoir si je peux payer le loyer en deux fois ce mois-ci ?",
      ...
      },
      {
        ...
        contenu:
          "Bonjour Paul, oui c'est possible. Faites-moi savoir quand vous comptez payer la première moitié.",
      ...
      },
      {
        ...
        destinataireNom: "Jean Kouassi",
        contenu:
          "Merci pour la réponse rapide ! Je paierai la première moitié demain.",
  ...
  envoyerMessage(): void {
    ...
    setTimeout(() => {
      const nouveauMessage: Message = {
        ...
        contenu: this.messageForm.getRawValue().contenu,
        date: new Date(),
        lu: false,
  ...
  archiverConversation(): void {
    if (this.conversationSelectionnee) {
      this.conversations = this.conversations.filter(
        (c) => c.id !== this.conversationSelectionnee!.id,
      );
  ...
  creerConversation(): void {
    ...
    const { locataireId, bienId } = this.newConversationForm.getRawValue();
