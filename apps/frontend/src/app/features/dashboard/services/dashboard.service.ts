...
export class DashboardService {
  ...
  ): Observable<RevenuMensuel[]> {
    return this.http
      .get<RevenuMensuel[]>(`${this.apiUrl}/revenus/${annee}`)
      .pipe(
        catchError((error: HttpErrorResponse) => {
  ...
  getDerniersPaiements(limit: number = 5): Observable<DernierPaiement[]> {
    return this.http
      .get<DernierPaiement[]>(`${this.apiUrl}/paiements/recent?limit=${limit}`)
      .pipe(
        catchError((error: HttpErrorResponse) => {
  ...
  getDerniersBiens(limit: number = 5): Observable<DernierBien[]> {
    return this.http
      .get<DernierBien[]>(`${this.apiUrl}/biens/recent?limit=${limit}`)
      .pipe(
        catchError((error: HttpErrorResponse) => {
  ...
  marquerAlerteLue(alerteId: string): Observable<void> {
    return this.http
      .patch<void>(`${this.apiUrl}/alertes/${alerteId}/lire`, {})
      .pipe(
        catchError((error: HttpErrorResponse) => {
