...
export class BiensListComponent implements OnInit {
  ...
  applyFilters(): void {
    this.filteredBiens = this.biens.filter((bien) => {
      ...
      if (this.recherche) {
        ...
        const matchVille = bien.adresse.ville
          .toLowerCase()
  ...
  getOccupesCount(): number {
    return (
      this.biens?.filter((b) => b.statut === StatutBien.OCCUPE).length || 0
    );
  }
  ...
  getVacantsCount(): number {
    return (
      this.biens?.filter((b) => b.statut === StatutBien.VACANT).length || 0
    );
  }
  ...
  getEnTravauxCount(): number {
    return (
      this.biens?.filter((b) => b.statut === StatutBien.EN_TRAVAUX).length || 0
    );
  }
  ...
  navigateToDetail(id: string): void {
    void this.router.navigate(["/dashboard/biens", id]);
  }
  ...
  navigateToEdit(id: string): void {
    void this.router.navigate(["/dashboard/biens", id, "edit"]);
  }
   ...
   */
  navigateToNew(): void {
    void this.router.navigate(["/dashboard/biens/nouveau"]);
  }
}
