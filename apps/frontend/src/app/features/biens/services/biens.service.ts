...
export class BiensService {
  ...
  changerStatut(id: string, statut: StatutBien): Observable<Bien> {
    return this.http
      .patch<Bien>(`${this.apiUrl}/${id}/statut`, { statut })
      .pipe(
        catchError((error: HttpErrorResponse) => {
   ...
   */
  getStatistiques(): Observable<BiensStatistiques> {
    return this.http.get<BiensStatistiques>(`${this.apiUrl}/statistiques`).pipe(
      catchError((error: HttpErrorResponse) => {
        console.error(
    ...
    return [
      {
        ...
        description:
          "Bel appartement de 3 chambres situé au cœur de Lomé, proche de tous les commerces.",
      ...
      },
      {
        ...
        titre: "Villa Sokodé",
        description:
          "Magnifique villa avec jardin et piscine, idéale pour une famille.",
      ...
      },
      {
        ...
        titre: "Studio Kara",
        description:
          "Studio moderne et fonctionnel, parfait pour un célibataire.",
      ...
      },
      {
        ...
        description:
          "Bureau spacieux dans un immeuble moderne, idéal pour une entreprise.",
  ...
  private getMockStatistiques() {
    ...
    return {
      total: biens.length,
      parType: {
        [TypeBien.APPARTEMENT]: biens.filter(
          (b) => b.typeBien === TypeBien.APPARTEMENT,
        ).length,
        [TypeBien.VILLA]: biens.filter((b) => b.typeBien === TypeBien.VILLA)
          .length,
        [TypeBien.STUDIO]: biens.filter((b) => b.typeBien === TypeBien.STUDIO)
          .length,
        [TypeBien.BUREAU]: biens.filter((b) => b.typeBien === TypeBien.BUREAU)
          .length,
        [TypeBien.LOCAL]: biens.filter((b) => b.typeBien === TypeBien.LOCAL)
          .length,
        [TypeBien.CHAMBRE]: biens.filter((b) => b.typeBien === TypeBien.CHAMBRE)
          .length,
      },
      parStatut: {
        [StatutBien.OCCUPE]: biens.filter((b) => b.statut === StatutBien.OCCUPE)
          .length,
        [StatutBien.VACANT]: biens.filter((b) => b.statut === StatutBien.VACANT)
          .length,
        [StatutBien.EN_TRAVAUX]: biens.filter(
          (b) => b.statut === StatutBien.EN_TRAVAUX,
        ).length,
        [StatutBien.ARCHIVE]: biens.filter(
          (b) => b.statut === StatutBien.ARCHIVE,
        ).length,
      },
      parVille: biens.reduce(
        (acc, bien) => {
