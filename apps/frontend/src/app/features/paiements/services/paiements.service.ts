...
export interface PaiementsStatistiques {
  total: number;
  totalMontant: number;
  payes: number;
  impayes: number;
...
export class PaiementsService {
  ...
  getPaiements(filters?: PaiementsFilters): Observable<Paiement[]> {
    ...
    if (filters) {
      ...
      if (filters.locataireId)
        params.append("locataireId", filters.locataireId);
   ...
   */
  getStatistiques(): Observable<PaiementsStatistiques> {
    return this.http
      .get<PaiementsStatistiques>(`${this.apiUrl}/statistiques`)
      .pipe(
        catchError((error: HttpErrorResponse) => {
          console.error(
  ...
  processMobileMoneyPayment(paymentData: {
    ...
    montant: number;
    periode: string;
  }): Observable<MobileMoneyPaymentResult> {
    return this.http
      .post<MobileMoneyPaymentResult>(
        `${this.apiUrl}/mobile-money`,
        paymentData,
      )
      .pipe(
        catchError((error: HttpErrorResponse) => {
  ...
  private getMockStatistiques() {
    ...
    const payes = paiements.filter(
      (p) => p.statut === StatutPaiement.PAYE,
    ).length;
    const impayes = paiements.filter(
      (p) => p.statut === StatutPaiement.IMPAYE,
    ).length;
    const enRetard = paiements.filter(
    ...
    return {
      ...
      enRetard,
      tauxRecouvrement:
        paiements.length > 0 ? (payes / paiements.length) * 100 : 0,
    };
  }
}
