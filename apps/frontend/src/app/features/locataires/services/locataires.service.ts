...
export interface LocatairesStatistiques {
  total: number;
  actifs: number;
  inactifs: number;
  enRetard: number;
...
export class LocatairesService {
  ...
  changerStatut(id: string, statut: StatutLocataire): Observable<Locataire> {
    return this.http
      .patch<Locataire>(`${this.apiUrl}/${id}/statut`, { statut })
      .pipe(
        catchError((error: HttpErrorResponse) => {
   ...
   */
  getStatistiques(): Observable<LocatairesStatistiques> {
    return this.http
      .get<LocatairesStatistiques>(`${this.apiUrl}/statistiques`)
      .pipe(
        catchError((error: HttpErrorResponse) => {
          console.error(
  ...
  private getMockStatistiques() {
    ...
    return {
      ...
      actifs: locataires.filter((l) => l.statut === StatutLocataire.ACTIF)
        .length,
      inactifs: locataires.filter((l) => l.statut === StatutLocataire.INACTIF)
        .length,
      enRetard: locataires.filter((l) => l.statut === StatutLocataire.EN_RETARD)
        .length,
    };
  }
}
